...
class StartSessionPragmaticPlay
{
    ...
    { 
        ...
        $buildArray = array(
            'game_id' => $selectGame->game_id,
            'game_name' => $selectGame->fullName,
            'game_id_original' => $selectGame->api_origin_id,
            'api_origin_id' => $selectGame->api_origin_id,
            'api_extension' => $selectGame->api_extension,
            'provider' => $strLowerProvider,
    ...
    {   
        ...
        if(!$getInternalSession) {
            ...
            $createInternalSession = Gamesessions::create([
                ...
                'game_id_original' => $request->api_origin_id,
        ...
        $replaceAPItoOurs = str_replace('https://demogamesfree.pragmaticplay.net/gs2c/common/js/html5-script-external.js', config('app.url').'/html5-script-external.js', $replaceAPItoOurs);

        $replaceAPItoOurs = str_replace('https://demogamesfree.pragmaticplay.net/gs2c/v3/gameService',  config('gameconfig.pragmaticplay.api_url').'/gs2c/v3/gameService', $replaceAPItoOurs);

        
        //$replaceAPItoOurs = str_replace('google-analytics.com/analytics.js', ' ', $replaceAPItoOurs);
